...
def resolve_fx_drilldown(table_id: str, row: pd.Series) -> FXDrilldownResolution | None:
    ...
    def result(measure: str, reason: str = "") -> FXDrilldownResolution:
        return FXDrilldownResolution(measure, grain, currency, box, subbucket, reason)
    ...
    if cell_id:
        ...
        if "Box" in spec.grain:
            grain = "box_currency"
            if not box:
                return result(contract_measure, "missing Box for Box x Currency FX contract")
        if len(spec.semantic_members) == 1:
            ...
            if subbucket and contract_subbucket and subbucket != contract_subbucket:
                return result(contract_measure, "semantic_subbucket conflicts with FX drilldown contract")
    ...
    if contract_measure and explicit and explicit != contract_measure:
        return result(explicit, "explicit measure conflicts with FX drilldown contract")

    metric = _text(row.get("metric")).casefold()
    measure = explicit or contract_measure or _TABLE_MEASURE.get(table_id, "") or _metric_measure(metric)
    if not measure:
        return result("", "ambiguous FX row has no explicit recognized measure")
    if not currency:
        return result(measure, "missing Currency would risk cross-currency aggregation")
...
def _fx_treasury_measure_for_row(table_id: str, row: pd.Series) -> str:
    ...
    return resolved.measure if resolved and resolved.measure in FX_MEASURES else ""
