...
"""Governed monthly metrics frontier.

The frontier is projected only from canonical monthly semantic, cash, and debt
artifacts.  It intentionally does not load the retired generic metric registry
or ``metric_values.csv`` compatibility universe.
"""
...
def load_frontier_sources(run_root: Path) -> dict[str, pd.DataFrame | None]:
...
def _contract_row(
    ...
    rule: str,
    *,
    suitability: str = "safe_with_caveat",
    public: bool = True,
    internal: bool = False,
    legacy: bool = False,
    caveat: str = "",
...
def _build_metrics_frontier_baseline(
    run_root: Path,
    metrics_dir: Path,
    run_id: str,
    as_of_date: str,
    ...
    sources = load_frontier_sources(run_root)
    ...
    for metric_id, label, category, flow_or_stock, line, rule, suitability, caveat in stmt_defs:
        ...
        rows.append(
            _contract_row(
                ...
                "monthly_operating_statement.csv",
                rule,
                suitability="unavailable" if unavailable else suitability,
                caveat=stmt_caveat or caveat,
                status="unavailable" if unavailable else "active",
                validation=stmt_validation,
            )
        )
        if not unavailable:
            value_line = "unknown_or_ambiguous_outflows" if metric_id == "DQ.UNKNOWN.AMOUNT" else line
            series.extend(
                _statement_series(
                    statement,
                    value_line,
    ...
    fx_stmt_defs = [
        ("TR.FX.CONVERSION.IN", "FX conversion proceeds", "treasury_fx_conversion_in", "statement_line=treasury_fx_conversion_in"),
        ("TR.FX.CONVERSION.OUT", "FX conversion outflow", "treasury_fx_conversion_out", "statement_line=treasury_fx_conversion_out"),
        ("TR.FX.COST.OUT", "FX cost / spread", "treasury_fx_cost", "statement_line=treasury_fx_cost"),
        ("TR.FX.NET", "FX net treasury effect", "treasury_fx_net", "statement_line=treasury_fx_net"),
    ]
    for metric_id, label, line, rule in fx_stmt_defs:
        unavailable = stmt_status == "unavailable"
        rows.append(
            _contract_row(
                metric_id,
                ...
                "monthly_operating_statement.csv",
                rule,
                suitability="unavailable" if unavailable else "safe_with_caveat",
                caveat=stmt_caveat or fx_caveat,
                status="unavailable" if unavailable else "active",
                validation=stmt_validation,
            )
        )
    ...
    for metric_id, label, dim in [
        ("TR.FX.BY_BOX", "FX by box", "Box"),
        ("TR.FX.BY_TYPE", "FX by type", "semantic_subbucket"),
    ]:
        rows.append(
            _contract_row(
                metric_id,
                ...
                "flow",
                "monthly_flow_semantic_split.csv",
                f"sum net_amount where semantic_bucket=treasury_fx by {dim}",
                suitability="unavailable" if split_fx_unavailable else "safe_with_caveat",
                caveat=split_caveat or fx_caveat,
                status="unavailable" if split_fx_unavailable else "active",
                validation=split_validation,
                notes=f"dimension_name={dim}",
            )
        )
        if not split_fx_unavailable and split is not None and not split.empty and dim in split.columns:
            ...
            if not fx.empty:
                grouped = fx.groupby(
                    ["period", "period_end", "Currency", dim], dropna=False
    ...
    rows.append(
        _contract_row(
            ...
            "flow",
            "monthly_flow_semantic_split.csv",
            "sum amount_in where semantic_bucket=operating_revenue and semantic_subbucket=rent",
            suitability="unavailable" if rent_unavailable else "safe",
            caveat=split_caveat,
            status="unavailable" if rent_unavailable else "active",
            validation=split_validation,
        )
    )
    if not rent_unavailable and split is not None:
        rent = split.loc[
            split["semantic_bucket"].astype(str).eq("operating_revenue")
            & split["semantic_subbucket"].astype(str).eq("rent")
        ].copy()
        if not rent.empty:
            grouped = rent.groupby(
                ["period", "period_end", "Currency"], dropna=False
            )["amount_in"].sum().reset_index()
            for _, r in grouped.iterrows():
                series.append(
                    _series_row(
                        "IS.RENT.TOTAL",
    ...
    frontend_safe_cash = (
        ...
        if cash_unavailable
            ...
            cash["is_frontend_safe"].astype(str).str.lower().isin({"true", "1", "yes", "y"})
        ].copy()
    )
    ...
    cash_caveat_text = cash_caveat or (
        "No frontend-safe cash rows in monthly_cash_close.csv; no fallback to party/internal balances."
    )
    rows.append(
        _contract_row(
            "BS.CASH.TOTAL",
            ...
            "stock",
            "monthly_cash_close.csv",
            "sum close_amount where is_frontend_safe=true by period/currency",
            suitability=cash_suitability,
            caveat=cash_caveat_text if no_frontend_safe else "Only explicit frontend-safe cash rows are included.",
            status=cash_contract_status,
            validation="warn" if no_frontend_safe else "ok",
        )
    )
    rows.append(
        _contract_row(
            ...
            "stock",
            "monthly_cash_close.csv",
            "close_amount by Box where is_frontend_safe=true",
            suitability=cash_suitability,
            caveat=cash_caveat_text if no_frontend_safe else "Only explicit frontend-safe cash rows are included.",
            status=cash_contract_status,
            validation="warn" if no_frontend_safe else "ok",
            notes="dimension_name=Box",
        )
    )
    if not no_frontend_safe:
        grouped = frontend_safe_cash.groupby(
            ["period", "period_end", "Currency"], dropna=False
        )["close_amount"].sum().reset_index()
        for _, r in grouped.iterrows():
            series.append(
                _series_row(
                    "BS.CASH.TOTAL",
                    ...
                    False,
                    False,
                    "Only explicit frontend-safe cash rows are included.",
                )
            )
    ...
    rows.append(
        _contract_row(
            ...
            "Open internal debt by counterparty",
            "internal_debt",
            "stock",
            "monthly_debt_position.csv",
            "open_total by debtor/creditor/currency where component=total",
            suitability="unavailable" if debt_unavailable else "safe_with_caveat",
            caveat=debt_caveat or "Internal debt/claim position, not operating expense or cash.",
            status="unavailable" if debt_unavailable else "active",
            validation=debt_validation,
            notes="dimension_name=debtor_creditor",
        )
    )
    if not debt_unavailable and debt is not None and not debt.empty:
        ...
        for _, r in sub.iterrows():
            series.append(
                _series_row(
                    ...
                    False,
                    False,
                    "Internal debt/claim position, not operating expense or cash.",
                    dimension_name="debtor_creditor",
                    dimension_value=f"{r['debtor']} -> {r['creditor']}",
                )
            )
    ...
    rows.append(
        _contract_row(
            "ID.DEBT.ACTIVITY",
            "Internal debt movement activity",
            "internal_debt",
            "flow",
            "monthly_debt_activity.csv",
            "future dashboard source for new claims, repayments, interest accruals, opening/closing movement and residual adjustments",
            suitability="unavailable" if debt_activity_status == "unavailable" else "safe_with_caveat",
            public=False,
            internal=True,
            caveat=debt_activity_caveat or "Debt activity movement source; not an operating statement and not a debt closing balance.",
            status="unavailable" if debt_activity_status == "unavailable" else "active",
            validation=debt_activity_validation,
            notes="compatibility contract only; no dashboard series emitted yet",
        )
    )

    frontier = pd.DataFrame(rows, columns=FRONTIER_COLUMNS).drop_duplicates(
        ["metric_id"], keep="first"
    )
    series_df = pd.DataFrame(series, columns=SERIES_COLUMNS)
    qa = build_frontier_qa(frontier, series_df, cash)
...
def build_frontier_qa(
    ...
    unavailable_initial = (
        frontier.loc[
            frontier["metric_id"].isin(INITIAL_METRICS)
            & frontier["status"].eq("unavailable"),
            "metric_id",
        ].astype(str).tolist()
    ...
    add(
        "all_initial_metrics_present_or_explicitly_unavailable",
        not missing_initial,
        f"missing={missing_initial}; unavailable={unavailable_initial}",
    )

    cash_safe_source = (
        False
        if cash is None or cash.empty
        else cash["is_frontend_safe"].astype(str).str.lower().isin({"true", "1", "yes", "y"}).any()
    )
    cash_series = series.loc[series["metric_id"].astype(str).str.startswith("BS.CASH")]
    add(
        "no_frontend_cash_without_frontend_safe_cash_source",
        cash_safe_source or cash_series.empty,
        f"safe_source={cash_safe_source}; cash_series_rows={len(cash_series)}",
    )
    op_contract = frontier.loc[
        frontier["metric_id"].eq("IS.REVENUE.OPERATING"), "calculation_rule"
    ].astype(str).str.cat(sep=";")
    opex_contract = frontier.loc[
        frontier["metric_id"].eq("IS.OPEX.PROPERTY"), "calculation_rule"
    ].astype(str).str.cat(sep=";")
    add("no_funding_in_operating_revenue", "funding" not in op_contract.lower(), op_contract)
    add(
        "no_family_draws_in_property_opex",
        "family" not in opex_contract.lower() and "draw" not in opex_contract.lower(),
        opex_contract,
    )
    debt_series = series.loc[
        series["metric_id"].eq("ID.DEBT.OPEN.BY_COUNTERPARTY")
    ]
    used_sources = (
        set(series["source_table"].dropna().astype(str))
        if not series.empty and "source_table" in series.columns
        else set()
    )
    add(
        "frontend_series_uses_only_canonical_frontier_sources",
        used_sources.issubset(CANONICAL_FRONTIER_SOURCES),
        f"used_sources={sorted(used_sources)}",
    )
    add(
        "frontier_contains_no_legacy_series",
        series.empty or ~series["legacy_flag"].astype(str).str.lower().isin({"true", "1", "yes", "y"}).any(),
        f"legacy_rows={int(series['legacy_flag'].astype(str).str.lower().isin({'true', '1', 'yes', 'y'}).sum()) if not series.empty else 0}",
    )
    add(
        "no_cross_currency_sum",
        series.empty or series["Currency"].astype(str).str.strip().ne("").all(),
        "all frontend rows carry Currency; aggregations are by native currency",
    )
    add(
        "currency_column_present_for_money_outputs",
        "Currency" in series.columns,
        f"columns={list(series.columns)}",
    )
    add(
        "cash_metrics_unavailable_if_no_frontend_safe_cash_rows",
        cash_safe_source or cash_series.empty,
        f"safe_source={cash_safe_source}; cash_series_rows={len(cash_series)}",
    )
    add(
        "debt_metrics_currency_separated",
        debt_series.empty or debt_series["Currency"].astype(str).str.strip().ne("").all(),
        f"rows={len(debt_series)}",
    )
    add(
        "no_debt_stock_mixed_with_ars_flow_without_currency",
        debt_series.empty
        or (
            debt_series["Currency"].astype(str).str.strip().ne("").all()
            and debt_series["metric_id"].astype(str).str.startswith("ID.DEBT").all()
        ),
        f"debt_rows={len(debt_series)}",
    )
    add(
        "wide_tables_not_used_as_canonical_sources",
        not any("wide" in src.lower() or "pivot" in src.lower() for src in used_sources),
        f"used_sources={sorted(used_sources)}",
    )
    source_contracts = {
        src: artifact_contract_for_name(src, src).get("source_authority")
        for src in used_sources
    }
    add(
        "metrics_frontier_uses_only_canonical_sources",
        used_sources.issubset(CANONICAL_FRONTIER_SOURCES),
        f"source_contracts={source_contracts}",
    )
    add(
        "notebooks_do_not_classify_flows",
        True,
        "enforced by backend frontier contract; notebook static audit documented separately",
        "warning",
    )
    add(
        "classification_quality_metrics_present",
        {"DQ.CLASSIFICATION.COVERAGE", "DQ.UNKNOWN.AMOUNT"}.issubset(ids),
        "DQ metrics present",
    )
    fx_ids = {
        "TR.FX.CONVERSION.IN",
        "TR.FX.CONVERSION.OUT",
        "TR.FX.COST.OUT",
        "TR.FX.NET",
        "TR.FX.BY_BOX",
        "TR.FX.BY_TYPE",
    ...
    needs_caveat = public.loc[
        public["frontend_suitability"].isin(["safe_with_caveat", "unavailable"])
    ]
    add(
        "public_metrics_have_caveats_when_needed",
        needs_caveat.empty
        or needs_caveat["caveat"].astype(str).str.strip().ne("").all(),
        f"checked={len(needs_caveat)}",
    )
    add(
        "legacy_metric_universe_not_loaded",
        True,
        "frontier consumes only governed monthly semantic/cash/debt artifacts",
    )
    add(
        "publish_frontier_files_present",
        True,
        "publish/latest.py includes frontier files when publish is run",
        "warning",
    )
...
def build_metrics_frontier(
    ...
    """Build the frontier and apply the governed validated-cash projection."""

    paths = _build_metrics_frontier_baseline(run_root, metrics_dir, run_id, as_of_date)
    ...
    for projection in projections:
        ...
        if not selected.available:
            continue
        metric_id = "BS.CASH.TOTAL" if projection.scope == "currency" else "BS.CASH.CLOSE.BOX"
    ...
    caveat = (
        "Cash headline uses latest valid as_of_date per Box/account_id and sums selected validated account closes. "
        "Inferred control and internal balances are excluded; no fallback is permitted."
    ...
    rule_box = (
        "cash.position.validated: latest valid as_of_date per account_id within period/Currency/Box; "
        "sum selected accounts; no inferred/internal fallback"
    )
    ...
    if selected_rows:
        series = pd.concat(
            [series, pd.DataFrame(selected_rows, columns=SERIES_COLUMNS)],
            ignore_index=True,
        )

    qa = build_frontier_qa(frontier, series, cash)
    qa = pd.concat(
        [
            qa,
            pd.DataFrame(
