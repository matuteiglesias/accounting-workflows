...
"""Metrics frontier with one authoritative implementation.

The module retains the historical/minimal-schema behavior for unsupported cash
inputs, while supported validated-cash inputs are rewritten from the shared
source-backed governed cash projection before artifacts are written.
"""
...
def _build_metrics_frontier_baseline(run_root: Path, metrics_dir: Path, run_id: str, as_of_date: str) -> Dict[str, Path]:
    run_root = Path(run_root)
...
def build_metrics_frontier(
    ...
    metric_registry = _read_csv(metrics_dir / "metric_registry.csv")
    metric_values = _read_csv(metrics_dir / "metric_values.csv")
    qa = build_frontier_qa(frontier, series, cash, metric_registry, metric_values)
    qa = pd.concat(
        [
            qa,
            pd.DataFrame(
