...
from src.accounting.utils import atomic_write_df
...
from src.accounting.utils import _read_csv_if_exists, _normalize_currency_col, _find_first_existing, _ensure_amount
import json
...
def export_views(reports_dir: Path, write_dir: Path, freq: str = "W") -> Dict[str, str]:
    ...
    if not party_consol.empty:
        # consolidated net / cum: sum across parties
        consol_net = party_consol.groupby("Date")["net"].sum().reset_index(name="net")
        consol_cum = party_consol.groupby("Date")["cum_net"].sum().reset_index(name="cum_net")
        consol = pd.merge(consol_net, consol_cum, on="Date")
        ...
        # also write consolidated wide (single time series) for quick dashboards
        # (we already have consolidated as single series; but keep it)
...
import argparse
import json
# from src.accounting.views import export_views # its here
