...
def resolve_paths(project_root: Path, public_subdir: Path) -> PublishPaths:
    out_root = project_root / "out"
    human_latest = (out_root / "human_reports" / "latest").resolve(strict=True)
    metrics_latest = (out_root / "metrics" / "latest").resolve(strict=True)
    debt_latest = (out_root / "debt_resolution" / "latest").resolve(strict=True)
...
def build_surface_manifest(paths: PublishPaths, report_info: dict[str, Any], metrics_info: dict[str, Any], debt_info: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    return {
        ...
        "publish_mode": mode,
        "run_id": build_manifest.get("run_id") or story_manifest.get("run_root", "").split("/")[-1] or None,
        "as_of_date": build_manifest.get("as_of_date"),
...
def main() -> None:
    args = parse_args()
    paths = resolve_paths(project_root=args.project_root, public_subdir=args.public_subdir)
