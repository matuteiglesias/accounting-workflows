...
from accounting.core_timeseries import process_time_aggregation
...
def read_sheet_to_df(sheet_url: str, service_account_file: str, sheet_name: str = "LEDGERS") -> pd.DataFrame:
    ...
    """
    try:
        from accounting.utils import get_google_sheets_client, load_google_sheet
...
# def process_time_aggregation(df, time_freq, date_col = 'Date'):
#     """
#     Ensures consistent time aggregation across all financial analyses.
    
#     Parameters:
#     - df: Pandas DataFrame (normalized ledger data)
#     - time_freq: str, either "Quarter", "Month", or "Week"
    
#     Returns:
#     - df: Updated DataFrame with a `TimePeriod` column for aggregation
#     """
...
#     freq_map = {
#         "D": "D",  # Daily
#         "W": "W-MON",  # Weekly (starts on Monday)
#         # "M": "MS",  # Month Start
#         "M": "M",  # Month Start
#         "Q": "QS",  # Quarter Start
#         "Y": "YS"  # Year Start
...
#     if time_freq not in freq_map:
#         raise ValueError("Invalid frequency. Choose from 'D', 'W', 'M', 'Q', 'Y'.") # freq=freq_map[time_freq]

#     # Generate start dates based on frequency
#     # df.loc[:, "TimePeriod"] = pd.to_datetime(df[date_col]).dt.to_period(freq_map[time_freq])
#     if "TimePeriod" in df.columns:
#         df = df.drop(columns=["TimePeriod"])
#     df["TimePeriod"] = pd.to_datetime(df[date_col]).dt.to_period(freq_map[time_freq])
...
def parse_args():
    ...
    p.add_argument("--mode", choices=["smoke", "run"], default=os.getenv("MODE"), help="Execution mode: smoke|run")
    p.add_argument("--run-id", default=os.getenv("RUN_ID"), help="Run identifier (timestamp or 'smoke'). Defaults to out-dir basename.")
...
def main():
    ...
    try:
        from accounting.ingest import build_ledger_base
    except Exception as e:
    ...
    # ----------------------------
    # Stage A manifest + artifacts
    # ----------------------------
    meta_dir = out_dir / "meta"
    meta_dir.mkdir(parents=True, exist_ok=True)

    # mode/run_id: prefer args, else infer from out_dir
    mode = (args.mode or ("smoke" if "smoke" in str(out_dir) else "run")).strip()
    run_id = (args.run_id or out_dir.name).strip()

    # artifacts for outputs
    artifacts = []

    ledger_art = artifact_from_path(
    ...
    artifacts.append(ledger_art)

    if anoms_path is not None:
        anoms_art = artifact_from_path(
            name="anomalies",
            path=anoms_path,
            stage="A.ingest",
            mode=mode,
        ...
        artifacts.append(anoms_art)

    # inputs (no es artifact, pero queda explícito)
    inputs = []
    if use_fixture:
        inputs.append({"kind": "fixture", "path": str(Path(fixture).expanduser().resolve())})
    else:
        inputs.append({"kind": "gsheet", "sheet_url": sheet_url, "sheet_name": args.sheet_name})

    stage_manifest = {
        "stage": "A.ingest",
        "mode": mode,
        "run_id": run_id,
        "generated_at": None,  # lo completa write_stage_manifest si falta
        "inputs": inputs,
        "params": {
    ...
    stage_meta_art = artifact_from_path(
        name="stage_A_ingest",
        path=(out_dir / stage_manifest_rel),  # stage_manifest_rel es relpath tipo "meta/stage_A_ingest.json"
        stage="A.ingest",
        mode=mode,
        run_id=run_id,
        role="meta",
    ...
    append_artifacts(meta_dir, artifacts + [stage_meta_art])
