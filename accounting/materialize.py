...
from src.accounting.core_timeseries import (
    aggregate_per_flow,
    expand_party_rows,
    aggregate_per_party,
...
def parse_args():
    ...
    p.add_argument("--out-dir", help="Output directory (where ledger_canonical.csv lives and where materialized CSVs will be written)", default=os.getenv("OUT_DIR", "./out"))
    p.add_argument("--freq", help="Aggregation frequency (W or M)", default=os.getenv("FREQ", "W"))
    p.add_argument("--force", help="Force rebuild (1 or 0)", default=os.getenv("FORCE", "0"))
    return p.parse_args()
...
def main():
    ...
    try:
        from src.accounting.materialize import materialize_all
    except Exception as e:
        LOG.exception("Failed importing materialize module: %s", e)
        sys.exit(10)
    ...
    try:
        ...
        LOG.exception("Failed reading ledger_canonical.csv: %s", e)
        sys.exit(3)
    print(f"check 0.1. Ledger path {ledger_path}, shape {ledger.shape}, columns: {ledger.columns}, count: {ledger.count()}")
...
# ledger
    print(f"check 0.2 shape {ledger.shape}, columns: {ledger.columns}, count: {ledger.count()}")

    try:
        manifest = materialize_all(ledger_df=ledger, out_dir=out_dir, freq=freq, loan_register_df=None, force=force_flag)
        print(manifest.get("generated_at", manifest))
    except Exception as e:
        LOG.exception("materialize_all failed: %s", e)
        sys.exit(5)
